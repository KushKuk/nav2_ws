...
def generate_launch_description():
    ...
    diff_drive_loaded_message = (
        "Successfully loaded controller drive_controller into state active"
    )
    ...
    waiting_toolbox = RegisterEventHandler(
        OnProcessIO(
            ...
            on_stdout=on_matching_output(
                ...
                [
                    LogInfo(
                        msg="Drive controller loaded. Starting SLAM Toolbox..."
                    ),
                    toolbox,
                ],
            ),
    ...
    return launch.LaunchDescription(
        [
            DeclareLaunchArgument(
                "params_file",
                default_value=[FindPackageShare("drive_nav2"), "/config/nav2_params.yaml"],
